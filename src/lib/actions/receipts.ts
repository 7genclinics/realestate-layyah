...
export async function receivePayment(input: unknown) {
  ...
  // "custom" is a UI-only sentinel for a hand-typed partial amount — it is
  // allocated the same way as auto (oldest open installment first).
  const targetInstallmentId =
    values.installment_id && values.installment_id !== "custom"
      ? values.installment_id
      : "";

  const ordered = targetInstallmentId
    ? [
        ...installments.filter((row) => row.id === targetInstallmentId),
        ...installments.filter((row) => row.id !== targetInstallmentId),
      ]
    : installments;
  ...
  const { data: receipt, error: receiptError } = await supabase
    ...
    .insert({
      ...
      // Only send slip_path when a slip is attached, so cash collections keep
  ...
  if (receiptError || !receipt) {
    if (values.slip_path) {
      ...
      if (/slip_path|column|schema cache/i.test(receiptError?.message ?? "")) {
        return {
          error:
